...
const InfoEnemyBullet = GetSpriteStaticInformation({ sprite: 'BigDiamondEnemy' }).spriteInfo.Bullet;
const scale = 3;
export class BigEnemyBullet
    extends Sprite
    implements ISpriteWithTarget, ISpriteWithDamage, ISpriteWithSpeed, ISpriteWithHitboxes, IGeneratedSprite
{
    ...
    constructor(parameters: { x: number; y: number; angleOffsetInRadian?: number }) {
        ...
        const BulletStats = GetSpriteStaticInformation({ sprite: 'BigDiamondEnemy' }).stats[roundTier - 1];
